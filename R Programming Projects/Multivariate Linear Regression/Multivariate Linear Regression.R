...
##
#######################################################################
setwd("C:/Users/91841/Desktop/BIG_DATA/Data_analytics/Assignment 4")##
getwd()                                                              ##  
#######################################################################
...
load("C:/Users/91841/Desktop/BIG_DATA/Data_analytics/Assignment 4/PROG8430_Assign_MLR_21F.RData")

print(PROG8430_Assign_MLR_21F)
typeof(PROG8430_Assign_MLR_21F)

MLR_data_pp<-as.data.frame(PROG8430_Assign_MLR_21F)
str(MLR_data_pp)
...
#3
###################################################################################
group_numeric_PP<-as.numeric(as.factor(PROG8430_Assign_MLR_21F$group))
hs.grad_numeric_PP<-as.numeric(as.factor(PROG8430_Assign_MLR_21F$hs.grad))
nation_numeric_PP<-as.numeric(as.factor(PROG8430_Assign_MLR_21F$nation))
gender_numeric_PP<-as.numeric(as.factor(PROG8430_Assign_MLR_21F$gender))
m.status_numeric_PP<-as.numeric(as.factor(PROG8430_Assign_MLR_21F$m.status))
political_numeric_PP<-as.numeric(as.factor(PROG8430_Assign_MLR_21F$political))
...
#Pleura
Tumor_data_pp$Pleura<-with(Tumor_data_pp,
                           ifelse(up_fix_pp(Tumor_data_pp$Pleura)<Pleura,up_fix_pp(Tumor_data_pp$Pleura),Pleura))

Tumor_data_pp$Pleura<-with(Tumor_data_pp,
                           ifelse(down_fix_pp(Tumor_data_pp$Pleura)>Pleura,down_fix_pp(Tumor_data_pp$Pleura),Pleura))
#Liver
Tumor_data_pp$Liver<-with(Tumor_data_pp,
                          ifelse(up_fix_pp(Tumor_data_pp$Liver)<Liver,up_fix_pp(Tumor_data_pp$Liver),Liver))

Tumor_data_pp$Liver<-with(Tumor_data_pp,
                          ifelse(down_fix_pp(Tumor_data_pp$Liver)>Liver,down_fix_pp(Tumor_data_pp$Liver),Liver))
#Skin
Tumor_data_pp$Skin<-with(Tumor_data_pp,
                         ifelse(up_fix_pp(Tumor_data_pp$Skin)<Skin,up_fix_pp(Tumor_data_pp$Skin),Skin))

Tumor_data_pp$Skin<-with(Tumor_data_pp,
                         ifelse(down_fix_pp(Tumor_data_pp$Skin)>Skin,down_fix_pp(Tumor_data_pp$Skin),Skin))
#Neck
Tumor_data_pp$Neck<-with(Tumor_data_pp,
                         ifelse(up_fix_pp(Tumor_data_pp$Neck)<Neck,up_fix_pp(Tumor_data_pp$Neck),Neck))

Tumor_data_pp$Neck<-with(Tumor_data_pp,
                         ifelse(down_fix_pp(Tumor_data_pp$Neck)>Neck,down_fix_pp(Tumor_data_pp$Neck),Neck))
#Supra
Tumor_data_pp$Supra<-with(Tumor_data_pp,
                          ifelse(up_fix_pp(Tumor_data_pp$Supra)<Supra,up_fix_pp(Tumor_data_pp$Supra),Supra))

Tumor_data_pp$Supra<-with(Tumor_data_pp,
                          ifelse(down_fix_pp(Tumor_data_pp$Supra)>Supra,down_fix_pp(Tumor_data_pp$Supra),Supra))
#Axil
Tumor_data_pp$Axil<-with(Tumor_data_pp,
                         ifelse(up_fix_pp(Tumor_data_pp$Axil)<Axil,up_fix_pp(Tumor_data_pp$Axil),Axil))

Tumor_data_pp$Axil<-with(Tumor_data_pp,
                         ifelse(down_fix_pp(Tumor_data_pp$Axil)>Axil,down_fix_pp(Tumor_data_pp$Axil),Axil))
